...
const Orders = () => {
    ...
    const updateStatus = useCallback(
        async (id) => {
            try {
                await axios.put(`${process.env.REACT_APP_BASE_URL}/orders/${id}`, {status: status});
                setStatus('');
                handleCloseModel();
        ...
        <div>
                        ...
                        <table className="w-full text-left">
                            ...
                            {currentOrders?.map((order) => (
                                ...
                                >
                                    ...
                                    <td className="px-2">{order.items.map((item) => item.ProductName).join(', ')}</td>
                                    <td className="px-2">{order.items.map((item) => item.Quantity).join(', ')}</td>
                                    <td className="px-2">{order.items.map((item) => item.Price).join(', ')}</td>
                                    <td className="px-2 capitalize text-center"> {order.TotalAmount}</td>
                                    {console.log(order.Status)}
                                    <td className="px-2 capitalize text-center">
                                        {order.Status === 'pending' ? (
                                            <span
                                                className="bg-yellow-500 text-white px-2 py-1 rounded-sm">{order.Status}</span>
                                        ) : order.Status === 'delivered' ? (
                                            <span
                                                className="bg-green-500 text-white px-2 py-1 rounded-sm">{order.Status}</span>
                                        ) : order.Status === 'processing' ? (
                                            <span
                                                className="bg-blue-500 text-white px-2 py-1 rounded-sm">{order.Status}</span>
                                        ) : (
                                            <span
                                                className="bg-red-500 text-white px-2 py-1 rounded-sm">{order.Status}</span>
                                        )}
                                    </td>
            ...
            {showModel && modelType === 'update' && (
                <Model onClose={handleCloseModel}>
                    <h2 className="min-w-[250px] text-lg font-bold text-gray-400 mb-2">Update Order Status</h2>
            ...
            {showModel && modelType === 'view' && (
                <Model onClose={handleCloseModel}>
                    <h2 className="min-w-[250px] md:min-w-[500px] text-lg font-bold text-gray-400 mb-2 text-center">Order
                        Details</h2>
                    <div className="w-full flex justify-center items-center">
                        <div className="w-full">
                            <div className="w-full text-left">
                                ...
                                <div className="w-full">
                                    <h2 className="text-lg mt-2 text-gray-400 text-center">Order Items</h2>
                                    <table
                                        className="w-full"
                                    >
                                        <thead>
                                        ...
                                        <tr
                                            className="text-gray-200 mt-2 border-t border-b border-t-gray-500 border-b-gray-500"
                                        >
                                            <th>
                                                Total
                                            </th>
